(function(){
  const VERSION='202609081150-natural-sun-v8';
  ...
  function naturalSunTarget(x,y,cur){
    const az=(cur.s.sunAz||180)*Math.PI/180;
    const alt=Math.max(0,Math.min(80,cur.s.sunAlt||0));
    let vx=Math.sin(az),vy=-Math.cos(az)*0.62-alt/180;
    const len=Math.hypot(vx,vy)||1;vx/=len;vy/=len;
    const radius=182-alt*.35;
    return {x:clamp(x+vx*radius,82,918),y:clamp(y+vy*radius-16,58,244)};
  }
  ...
  function install(){
    ...
    window.drawAnimated=function(segs){
      ...
      const sunGlow=svgEl('filter',{id:'sunGlow',x:'-150%',y:'-150%',width:'400%',height:'400%'}),sunBlur=svgEl('feGaussianBlur',{stdDeviation:'20'});sunGlow.appendChild(sunBlur);defs.append(glow,sunGlow);svg.appendChild(defs);
    ...
    window.renderAnimation=function(p){
      ...
      const x=cur.xy[0],y=cur.xy[1],target=naturalSunTarget(x,y,cur);
      if(SUN_STATE.x==null||SUN_STATE.y==null||ANIM.progress<.015){SUN_STATE.x=target.x;SUN_STATE.y=target.y}else{SUN_STATE.x=lerp(SUN_STATE.x,target.x,.10);SUN_STATE.y=lerp(SUN_STATE.y,target.y,.10)}
      const sunX=SUN_STATE.x,sunY=SUN_STATE.y;
      const train=$('train-marker'),shadow=$('train-cast-shadow'),core=$('sun-core'),halo=$('sun-halo'),ray=$('sun-ray'),prog=$('route-progress');
      train?.setAttribute('transform',`translate(${x} ${y}) rotate(${cur.angle}) scale(1.24)`);core?.setAttribute('cx',sunX);core?.setAttribute('cy',sunY);halo?.setAttribute('cx',sunX);halo?.setAttribute('cy',sunY);
      const vx=x-sunX,vy=y-sunY,len=Math.hypot(vx,vy)||1,ux=vx/len,uy=vy/len,rayGap=46;
      ray?.setAttribute('x1',sunX+ux*rayGap);ray?.setAttribute('y1',sunY+uy*rayGap);ray?.setAttribute('x2',x);ray?.setAttribute('y2',y);
      if(shadow){const sx=x+ux*29,sy=y+uy*21+7;shadow.setAttribute('cx',sx);shadow.setAttribute('cy',sy);shadow.setAttribute('transform',`rotate(${Math.atan2(uy,ux)*180/Math.PI} ${sx} ${sy})`)}
